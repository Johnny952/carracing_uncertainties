...
class DropoutAgent(AbstactAgent):
    def __init__(
        self,
        nb_nets,
        img_stack,
        ...
        self._optimizer1 = torch.optim.Adam(self._model1.parameters(), lr=self._lr, weight_decay=self.weight_decay)
        self._optimizer2 = torch.optim.Adam(self._model2.parameters(), lr=self._lr, weight_decay=self.weight_decay)
    
    def get_values(self, observation):
        tau = self.lengthscale * (1. - self.prob) / \
            (2. * observation.shape[0] * self.weight_decay)
        values_list = []
