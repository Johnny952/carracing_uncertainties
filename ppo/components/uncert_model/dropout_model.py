...
class DropoutTrainerModel(BaseTrainerModel):
    def __init__(self, nb_nets, lr, img_stack, gamma, batch_size, buffer_capacity, device='cpu'):
        super(DropoutTrainerModel, self).__init__(nb_nets, lr, img_stack, gamma, batch_size, buffer_capacity, device=device)
    ...
    def get_uncert(self, state):
        ...
        tau = self.lengthscale * (1. - self.prob) / (2. * state.shape[0] * self.weight_decay)
