...
def plot_uncert_train(paths, names, colors=None, linewidths=None, unc_path='images/uncertainties_train.png', rwd_path='images/rewards_train.png', smooth=None):
    ...
    for idx, (path, name) in enumerate(zip(paths, names)):
        ...
        _, (unique_ep, mean_reward, mean_epist, mean_aleat), (std_reward, std_epist, std_aleat), _ = read_uncert(path)
...
def plotly_train(paths, names, colors=None, save_fig='images/uncertainties_train.html', smooth=None):
    fig = make_subplots(
        rows=2, cols=2,
        specs=[[{}, {}],
            [{"colspan": 2}, None]],
...
def plot_uncert_test(paths, names, colors=None, linewidths=None, unc_path='images/uncertainties_test.png', rwd_path='images/rewards_test.png', smooth=None):
    ...
    for idx, (path, name) in enumerate(zip(paths, names)):
        color = colors[idx] if colors is not None else None
        linewidth = linewidths[idx] if linewidths is not None else None
        (_, (_, mean_reward, mean_epist, mean_aleat), (std_reward, std_epist, std_aleat), _), sigma = read_uncert_test(path)
        if smooth is not None:
            mean_reward = gaussian_filter1d(mean_reward, smooth)
            mean_epist = gaussian_filter1d(mean_epist, smooth)
            mean_aleat = gaussian_filter1d(mean_aleat, smooth)
        ...
        # Plot rewards
        ax_rwd.plot(sigma, mean_reward, color, label="Mean " + name, linewidth=linewidth)
        ax_rwd.fill_between(sigma, (mean_reward - std_reward), (mean_reward + std_reward), color=color, alpha=0.2, label="Std " + name)
...
def plotly_test(paths, names, colors=None, save_fig='images/uncertainties_test.html', smooth=None):
    fig = make_subplots(
        rows=2, cols=2,
        specs=[[{}, {}],
            [{"colspan": 2}, None]],
    ...
    fig.update_xaxes({"title": {"text": "Noise Variance", "font": {"size": 20}}}, row=1, col=1)
    fig.update_xaxes({"title": {"text": "Noise Variance", "font": {"size": 20}}}, row=1, col=2)
    fig.update_xaxes({"title": {"text": "Noise Variance", "font": {"size": 20}}}, row=2, col=1)
